...
public class Perceptron
{
   ...
   {
      double capitalOmega, capitalPsi, deltaWeights;
      for (int n = 1; n < noLayers; n++)
      {
         for (int j = 0; j < noActivations[n]; j++)
         {
            ...
            for (int i = 0; i < noActivations[noLayers]; i++)
            {
               ...
               deltaWeights = lambda * activations[n][j] * psi[i];
            ...
            {
               deltaWeights = lambda * activations[n - 1][k] * capitalPsi;
               weights[n - 1][k][j] += deltaWeights;
            }
         } // for (int j = 0; j < noActivations[n]; j++)
      } // for (int n = 1; n < noLayers; n++)
